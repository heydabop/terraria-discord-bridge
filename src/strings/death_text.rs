use std::collections::HashMap;

pub fn get() -> HashMap<&'static str, &'static str> {
    [
        ("Suffocated_1", "{0} couldn't breathe."),
        ("Suffocated_2", "{0} was buried alive."),
        ("Poisoned", "{0} couldn't find the antidote."),
        ("Burned_1", "{0} couldn't put the fire out."),
        ("Burned_2", "{0} was reduced to charcoal."),
        ("Burned_3", "{0} was burnt to a crisp."),
        ("Burned_4", "{0} is a well-done steak."),
        ("Electrocuted_1", "{0} couldn't contain the watts."),
        ("Electrocuted_2", "{0} was turned into a battery."),
        (
            "Electrocuted_3",
            "{0}'s positive lifeforce became negative.",
        ),
        ("Electrocuted_4", "{0} became a lightning rod."),
        ("TriedToEscape", "{0} tried to escape."),
        ("WasLicked_1", "{0} was licked."),
        ("WasLicked_2", "{0} got to 1st base with the Wall of Flesh!"),
        ("Teleport_1", "{0} didn't materialize"),
        (
            "Teleport_2_Male",
            "{0}'s legs appeared where their head should be.",
        ),
        (
            "Teleport_2_Female",
            "{0}'s legs appeared where their head should be.",
        ),
        ("Slain", "{0} was slain..."),
        ("Stabbed", "{0} was stabbed."),
        ("Default", "{0}."),
        ("Fell_1", "{0} fell to their death."),
        ("Fell_2", "{0} didn't bounce."),
        ("Fell_3", "{0} invented gravity."),
        ("Fell_4", "{0} discovered the meaning of defenestration."),
        ("Fell_5", "{0} was freeeee, free-fallin'."),
        ("Fell_6", "{0} tried to ice skate uphill."),
        ("Fell_7", "{0} thought they could fly."),
        ("Fell_8", "{0} left a crater."),
        ("Fell_9", "{0} forgot their happy thought."),
        ("Drowned_1", "{0} forgot to breathe."),
        ("Drowned_2", "{0} is sleeping with the fish."),
        ("Drowned_3", "{0} drowned."),
        ("Drowned_4", "{0} is shark food."),
        ("Drowned_5", "{0} tried to drink a lake."),
        ("Drowned_6", "{0} discovered Atlantis."),
        ("Drowned_7", "{0} forgot to bring a towel."),
        ("Lava_1", "{0} got melted."),
        ("Lava_2", "{0} was incinerated."),
        ("Lava_3", "{0} tried to swim in lava."),
        ("Lava_4", "{0} likes to play in magma."),
        ("Lava_5", "{0} is bad at the Floor Is Lava."),
        ("Petrified_1", "{0} shattered into pieces."),
        ("Petrified_2", "{0} can't be put back together again."),
        ("Petrified_3", "{0} needs to be swept up."),
        ("Petrified_4", "{0} just became another dirt pile."),
        ("Inferno", "{0} was consumed by the inferno."),
        ("DiedInTheDark", "{0} was killed by something in the dark!"),
        ("Starved_1", "{0} starved to death."),
        ("Starved_2", "{0} couldn't find food."),
        ("Starved_3", "{0} forgot to eat."),
        ("Space_1", "{0} became an astronaut."),
        ("Space_2", "{0} is now space debris."),
        ("Space_3", "{0} left orbit."),
        ("Space_4", "{0} has ascended."),
        ("Space_5", "{0} departed {1}."),
    ]
    .iter()
    .copied()
    .collect()
}
